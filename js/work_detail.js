document.addEventListener("DOMContentLoaded", function () {
    ...
    const pagination = paginationNav ? paginationNav.querySelector(".pagination") : null;
  
    if (!form || !commentList || !pagination) {
      console.warn("留言表單或留言列表或分頁導航未正確加載，請檢查 HTML 是否正確");
      return;
    }
  
    const portfolioIdInput = form.querySelector("[name='portfolio_id']");
    const portfolioId = portfolioIdInput ? portfolioIdInput.value : null;
    let currentPage = 1;
  
    if (!portfolioId) {
      console.warn("找不到 portfolio_id，無法載入留言");
      return;
    }
